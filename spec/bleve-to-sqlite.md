...
* Create Init() method on DictionaryStore and ExcerptStore, which create necessary schemas/mappings.

#### Step 2: SQLite3 scaffolding and data load
* Replace bleve dependencies which are being injected by SQLite3
* Make it possible to switch between mattn sqlite3 (CGo based) and ModernC sqlite3 using a build tag in one file.
...
#### Step 3 Implement store methods in `DictionaryStore` and `ExcerptStore`.
  * Mirror the existing Bleve implementations in same file. (We will keep bleve implementation for now).
